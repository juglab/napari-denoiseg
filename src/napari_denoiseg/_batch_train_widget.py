...
def get_load_images_button(Images: Path):
    pass


@magic_factory(auto_call=True, Labels={'mode': 'd'})
def get_load_labels_button(Labels: Path):
    pass


class BatchTrainWidget(QWidget):
    def __init__(self, napari_viewer):
        ...
        self.images = None
        self.labels = None
        self.load_images_button = get_load_images_button()
        self.load_labels_button = get_load_labels_button()
        self.layout().addWidget(self.load_images_button.native)
        self.layout().addWidget(self.load_labels_button.native)
...
def train_worker(widget: BatchTrainWidget):
    import os
    import threading
    ...
    # TODO check if the data for the labels is an int
    img_paths = [widget.load_images_button.Images.value, widget.load_labels_button.Labels.value]
    images = []
    for d in img_paths:
        imgs = []
    ...
    widget.images = images[0]
    widget.labels = images[1].astype(np.intc)
    image_data = widget.images
    label_data = widget.labels
...
if __name__ == "__main__":
    ...
    # add our plugin
    viewer.window.add_dock_widget(BatchTrainWidget(viewer))
